...
const FailedPayments: React.FC = () => {
  const [selectedPayments, setSelectedPayments] = useState<FailedPaymentRecord[]>([]);

  const { data: payments, isLoading, error } = useQuery({
    queryKey: ['failed-payments'],
    queryFn: () => rydoraApi.getPayments('failed')
  });
  ...
  useEffect(() => {
    if (error) {
      ...
      if ((error as any).response?.status !== 404) {
        toast.error((error as any).response?.data?.message || 'Failed to load failed payments');
      }
    }
  }, [error]);
  ...
  const handleRetrySelected = () => {
    if (selectedPayments.length === 0) {
      toast.error('Please select payments to retry.');
      return;
    }
    toast.success(`Retrying ${selectedPayments.length} payment(s)...`);
  ...
  const handleExportExcel = async () => {
    ...
    const success = exportToExcel(
      payments.data, 
      `Failed_Payments_Data_${new Date().toISOString().slice(0,19).replace(/:/g, '-')}`
    );
  ...
  const handleExportPdf = async () => {
    ...
    try {
      ...
      // Add title
      doc.setFontSize(18);
      doc.text('Failed Payments', 14, 20);
      ...
      // Save the PDF
      const filename = `Failed_Payments_${new Date().toISOString().slice(0,19).replace(/:/g, '-')}`;
  ...
  const columns = useMemo<ColumnDef<FailedPaymentRecord>[]>(
    () => [
      {
      ...
      },
      {
        ...
        cell: ({ getValue }) => {
          ...
          return <span className="text-danger fw-bold">${amount.toFixed(2)}</span>;
        },
      },
      {
  ...
  if (isLoading) {
    return (
      <div className="text-center py-4">
        ...
        <p className="mt-2">Loading failed payments...</p>
      </div>
    );
  }
    ...
    <section className="authenticated-page">
      <div className="container">
        <div style={{ display: 'flex', gap: '16px', alignItems: 'center', justifyContent: 'flex-start', flexWrap: 'wrap', marginBottom: '16px', width: '100%' }}>
          <h2 style={{ margin: '0' }}>Failed Payments</h2>
        </div>
        ...
        {payments?.data && payments.data.length > 0 ? (
          <DataTable
            data={payments.data as FailedPaymentRecord[]}
            columns={columns}
            searchable={true}
            exportable={true}
